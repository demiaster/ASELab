...
NGLScene::NGLScene()
{
  ...
  setTitle("Blank NGL");
  std::cout<<m_view<<'\n'<<m_projection<<'\n';
  m_view=ngl::lookAt(ngl::Vec3(2.0f,2.0f,5.0f),
                     ngl::Vec3::zero(),
                     ngl::Vec3::up());
  std::cout<<"view \n"<<m_view<<'\n';
...
void NGLScene::resizeGL(int _w , int _h)
{
  ...
  m_projection=ngl::perspective(45.0f,
                               static_cast<float>(_w)/_h,
                                0.5f,
                                20.0f);
  std::cout<<"projection"<<m_projection<<'\n';
}
  ...
  // now for the steps in w and d
   ...
   for (ngl::Real d=-d2; d<d2; d+=dStep)
   {
       for (ngl::Real w=-w2; w<w2; w+=wStep)
       {
          ...
          //second triangle
          vert.m_x=w+wStep;   vert.m_z=d+dStep;  ///V1
          data.push_back(vert);
          vert.m_x=w+wStep;        vert.m_z=d;   ///V-
          data.push_back(vert);
          vert.m_x=w;        vert.m_z=d;         ///V3
          data.push_back(vert);
